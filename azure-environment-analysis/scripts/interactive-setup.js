...
console.log(chalk.bold.cyan('║  Azure Integration Assessment - Setup Wizard          ║'));
console.log(chalk.bold.cyan('╚════════════════════════════════════════════════════════╝\n'));
...
async function promptAuthentication() {
  ...
  if (currentAccount) {
    ...
    if (authChoice === 'help') {
      ...
      console.log(chalk.cyan('  az ad sp create-for-rbac --name "azure-assessment-sp" \\'));
      console.log(chalk.cyan('    --role Reader \\'));
      console.log(chalk.cyan('    --scopes /subscriptions/{subscription-id}\n'));
...
async function validateAndDiscoverSubscriptions(authInfo) {
  ...
  try {
    ...
    console.log(chalk.cyan(`✓ Found ${subscriptions.length} subscription(s)\n`));

    if (subscriptions.length === 0) {
      console.log(chalk.yellow('⚠️  No subscriptions found. Your account may lack access.\n'));
      process.exit(1);
    }
    ...
    if (subscriptions.some(s => s.accessLevel !== 'Reader' && s.accessLevel !== 'Contributor')) {
      console.log(chalk.yellow('⚠️  Note: Reader access is sufficient for assessment.'));
      console.log(chalk.yellow('   Contributor recommended for remediation actions.\n'));
    }
    ...
    if (authInfo.type === 'sp') {
      ...
      console.log('  • Service principal has been granted access to subscriptions');
...
async function promptAssessmentScope() {
  ...
  const scope = await inquirer.prompt([
    ...
    },
    {
      ...
      default: 30,
      validate: (input) => input > 0 && input <= 90 || 'Must be between 1 and 90 days'
    },
    {
      type: 'checkbox',
      name: 'focusAreas',
      ...
      choices: [
        ...
        { name: 'Logic Apps analysis', value: 'logic-apps-deep-dive', checked: true },
        ...
        { name: 'Naming/tagging compliance', value: 'naming-tagging', checked: true },
        { name: 'Sales opportunities (internal only)', value: 'sales-opportunities', checked: false }
      ]
    }
  ]);
...
async function promptCredentialStorage(credentials) {
  ...
  const { storageMethod } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: 'Encrypted file (~/.azure-assessment/credentials.enc)', value: 'encrypted' },
        { name: 'Azure Key Vault (requires additional setup)', value: 'keyvault' }
      ]
    }
  ]);
  ...
  if (storageMethod === 'env') {
    ...
    console.log(chalk.gray(`$env:AZURE_CLIENT_SECRET = "${credentials.clientSecret}"`));
    ...
    console.log(chalk.gray(`export AZURE_CLIENT_SECRET="${credentials.clientSecret}"`));
...
async function generateClientConfig(authInfo, selectedSubscriptions, clientConfig, scope, standards, storage) {
  const clientSlug = clientConfig.clientName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  ...
  const config = {
    ...
    azureAccess: {
      ...
      authenticationType: authInfo.type === 'sp' ? 'service-principal' : 'azure-cli',
      ...(authInfo.type === 'sp' ? {
        servicePrincipal: {
          clientId: authInfo.clientId,
          credentialSource: storage.storageMethod,
          notes: storage.storageMethod === 'env'
        ...
        notes: "Using Azure CLI interactive authentication. Ensure az login session is active before running assessments."
      }),
      accessLevel: "minimum",
  ...
  const outputDir = path.join(PROJECT_ROOT, 'output', clientSlug);
  await fs.mkdir(path.join(outputDir, 'inventory'), { recursive: true });
  await fs.mkdir(path.join(outputDir, 'analysis', 'logic-apps'), { recursive: true });
...
async function testMCPConnectivity() {
  ...
  console.log(chalk.yellow('  ⚠ Logic Apps MCP: Known auth issue, using Azure CLI fallback'));
  console.log(chalk.cyan('  ✓ Microsoft Docs MCP: Will be available in agent'));
  console.log(chalk.cyan('  ✓ Atlassian MCP: Will be available in agent (requires ATLASSIAN_API_TOKEN)'));
...
async function main() {
  try {
    ...
    const { credential, subscriptions } = await validateAndDiscoverSubscriptions(authInfo);
    ...
    console.log(chalk.yellow(`  ${standards.fetchFromConfluence ? '3' : '2'}. Validate setup:`));
    console.log(chalk.gray('     npm run validate'));

    console.log(chalk.yellow(`  ${standards.fetchFromConfluence ? '4' : '3'}. Open START-HERE.md for assessment instructions\n`));

    console.log(chalk.gray('For automated/CI mode, set environment variables and run:'));
    console.log(chalk.gray(`  npm run validate\n`));
