...
console.log(
  chalk.bold.cyan("║           MCP Server Connectivity Test                ║"),
);
console.log(
  chalk.bold.cyan(
    "╚════════════════════════════════════════════════════════╝\n",
...
console.log(
  chalk.bold(
    "MCP servers are initialized by VS Code when you open Copilot Chat.\n",
  ),
);
...
console.log(
  chalk.gray("    Tools: Resource management, RBAC, Monitor, Log Analytics\n"),
);
...
console.log(
  chalk.gray(
    "    Requires: ATLASSIAN_API_TOKEN and ATLASSIAN_USER_EMAIL env vars",
  ),
);
...
const requiredEnvVars = {
  ...
  "Atlassian Confluence": ["ATLASSIAN_API_TOKEN", "ATLASSIAN_USER_EMAIL"],
};
...
const allAtlassianSet =
  process.env.ATLASSIAN_API_TOKEN && process.env.ATLASSIAN_USER_EMAIL;

if (!allAzureSet) {
  console.log(
    chalk.yellow("⚠️  Azure credentials not set. Run: npm run setup\n"),
  );
}
...
console.log("  4. Read START-HERE.md for assessment guide\n");
