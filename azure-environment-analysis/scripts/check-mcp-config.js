...
const USER_MCP_CONFIG_PATH = path.join(
  os.homedir(),
  "AppData",
  "Roaming",
  "Code",
...
const USER_MCP_CONFIG_PATH_MAC = path.join(
  os.homedir(),
  "Library",
  "Application Support",
  "Code",
...
async function checkMCPConfiguration() {
  ...
  if (!configPath) {
    ...
    console.log(
      chalk.yellow(
        "If you haven't set up MCP servers yet, install them in VS Code first.\n",
      ),
    );
    return false;
  }
  ...
  if (results.missing.length > 0) {
    console.log(
      chalk.red(`  ✗ ${results.missing.length} required MCP servers missing`),
    );
  }
  ...
  if (results.optional.length > 0) {
    console.log(
      chalk.yellow(
        `  ⚠ ${results.optional.length} optional MCP servers not found`,
      ),
    );
  }
  ...
  console.log(
    chalk.cyan('  "Use Atlassian MCP to fetch SSOT standards from Confluence"'),
  );
  console.log(
    chalk.cyan(
      '  "Use Microsoft Docs MCP to find Azure Logic Apps best practices"',
    ),
  );
  console.log(
    chalk.cyan(
      '  "Use Azure MCP to list all Logic Apps in the subscription"\n',
    ),
  );

  console.log(
    chalk.gray(
      "The AI agent will automatically use the appropriate MCP server.\n",
    ),
  );
