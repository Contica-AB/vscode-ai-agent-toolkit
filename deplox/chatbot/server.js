...
// ── Azure Retail Prices API — cost estimation ─────────────────────────────────
...
async function queryAzurePrices(filters) {
  try {
...
function findHourlyMeter(items, sku, pattern) {
  return items.find(i =>
...
async function estimateMonthlyCost(service, collected, location) {
  ...
  try {
    switch (service) {
      ...
      case 'logicapp-standard': {
        // WS1=1 vCPU + 3.5 GB, WS2=2 vCPU + 7 GB, WS3=4 vCPU + 14 GB
      ...
      case 'integrationaccount': {
        ...
        const items = await queryAzurePrices([`serviceName eq 'Logic Apps'`, ...base]);
        // API meters are named "Basic Unit", "Standard Unit", "Premium Unit" under Logic Apps
        const m = items.find(i => i.skuName?.toLowerCase() === sku.toLowerCase()
...
function formatCostEstimate(est) {
  if (!est) return '';
